...
import {
  agregarVolumeNotasPorBairro,
  type BairroVolumeAgg,
} from "@/components/KpiDetalhamentoNotas";
...
const TECNICO_NENHUM = "__selecione__";
...
function listarTecnicosUnicos(rows: ToaImportacaoRow[]): string[] {
  const set = new Set<string>();
  for (const row of rows) {
    set.add(nomeTecnicoDaLinha(row));
  }
  return [...set].sort((a, b) => a.localeCompare(b, "pt-BR"));
}

export function KpiNotaPorTecnico() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        if (cancelled) return;
        console.error("Erro ao carregar nota por técnico:", err);
        setError(
          err instanceof Error
            ? err.message
            : "Não foi possível carregar as notas TOA.",
        );
        setRows([]);
      } finally {
  ...
  const tecnicosDisponiveis = useMemo(
    () => listarTecnicosUnicos(rows),
    [rows],
  );

  useEffect(() => {
    if (tecnicosDisponiveis.length === 0) {
      setTecnico(TECNICO_NENHUM);
      return;
    }
    if (!tecnicosDisponiveis.includes(tecnico)) {
      setTecnico(tecnicosDisponiveis[0]!);
    }
  ...
  const rowsDoTecnico = useMemo(() => {
    if (tecnico === TECNICO_NENHUM) return [];
    return rows.filter((row) => nomeTecnicoDaLinha(row) === tecnico);
  }, [rows, tecnico]);

  const porBairro: BairroVolumeAgg[] = useMemo(
  ...
  const periodoDescricao = useMemo(() => {
    const tec =
      tecnico === TECNICO_NENHUM ? "nenhum técnico" : tecnico;
    if (filtrosLimpos) return `Histórico completo TOA · ${tec}`;
    if (ano !== null && mes !== null) {
      return `${mesLabel(mes)} de ${ano} · ${tec}`;
    }
    if (ano !== null) return `Ano ${ano} · todos os meses · ${tec}`;
    return tec;
  }, [filtrosLimpos, ano, mes, tecnico]);
  ...
  const semTecnico = tecnico === TECNICO_NENHUM;
    ...
    <div className="space-y-6">
      <div className="rounded-xl border border-border bg-background px-4 py-3 shadow-sm">
        <div className="flex flex-row flex-wrap items-center gap-4 sm:gap-6">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <SelectTrigger id="nota-tec-ano" className="w-[140px]">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <SelectTrigger id="nota-tec-mes" className="w-[160px]">
                <SelectValue placeholder="Todos" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="todos">Todos</SelectItem>
          ...
          <div className="flex items-center gap-2">
            ...
            <Select
              value={tecnico}
              disabled={loading || tecnicosDisponiveis.length === 0}
              onValueChange={setTecnico}
            >
              <SelectTrigger id="nota-tec-tecnico" className="w-[240px]">
                <SelectValue placeholder="Selecione um técnico" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value={TECNICO_NENHUM}>
      ...
      {loading ? (
        ...
        <p className="rounded-lg border border-border bg-muted/40 px-4 py-8 text-center text-sm text-muted-foreground">
          Selecione um técnico para ver a análise geográfica individual.
        </p>
      ) : (
        <>
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
            <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
              ...
              {chartProdutivas.length === 0 ? (
                <p className="flex h-64 items-center justify-center text-sm text-muted-foreground">
                  Nenhuma nota produtiva deste técnico no período.
                </p>
              ) : (
            ...
            <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
              ...
              {chartImprodutivas.length === 0 ? (
                <p className="flex h-64 items-center justify-center text-sm text-muted-foreground">
                  Nenhuma nota improdutiva deste técnico no período.
                </p>
              ) : (
                <div className="h-80 w-full">
                  <ResponsiveContainer width="100%" height="100%">
