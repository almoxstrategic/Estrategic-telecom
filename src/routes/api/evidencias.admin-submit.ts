...
async function assertTecnico(tecnicoId: string) {
...
export const Route = createFileRoute("/api/evidencias/admin-submit")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        try {
          ...
          if (error) {
            await supabase.storage.from(EVIDENCIAS_BUCKET).remove([inicio.path, fim.path]);
