...
type PopoverView = "selecao" | "definicao";
...
/** ~70% das linhas com (Físico + Campo) < BTP para favorecer status Falta. */
function gerarQuantidadesMock(): Pick<
  LinhaMockEstoque,
  "estoqueBTP" | "estoqueFisico" | "estoqueCampo" | "diferenca"
> {
  ...
  return {
    ...
    estoqueFisico,
    estoqueCampo,
    diferenca: estoqueFisico + estoqueCampo - estoqueBTP,
  };
}
...
function EstoqueFisicoBtpPage() {
  ...
  const [viewAtual, setViewAtual] = useState<PopoverView>("selecao");
  ...
  const linhasFiltradas = useMemo(() => {
    ...
    const filtradas = linhas.filter((row) => {
      ...
      if (itensSelecionados.length > 0) {
        return itensSelecionados.includes(row.codigo);
      }
      return true;
    });
  ...
  const fecharModal = () => handlePopoverOpenChange(false);
  ...
  if (viewAtual === "definicao") {
      ...
      <>
        <div className="shrink-0 border-b bg-white p-4">
          <div className="flex items-start justify-between gap-2">
            <div>
              <p className="text-sm font-semibold text-foreground">Definir itens críticos</p>
    ...
    popoverBody = (
      <>
        <div className="shrink-0 border-b bg-white p-4">
          <div className="flex items-start justify-between gap-2">
            <p className="text-sm font-semibold text-foreground">Selecionar itens</p>
            <button
              type="button"
              aria-label="Fechar"
              className="rounded-md p-1 text-muted-foreground hover:bg-accent hover:text-foreground"
    ...
    <div className="min-h-screen bg-surface">
      ...
      <main className="mx-auto min-h-[80vh] max-w-7xl px-5 pb-10 pt-6">
        ...
        <div className="mb-4 flex flex-wrap items-center gap-2">
          ...
          <Popover open={isModalAberto} onOpenChange={handlePopoverOpenChange}>
            ...
            <PopoverContent
              ...
              className="z-[100] flex w-[min(100vw-2rem,24rem)] flex-col overflow-hidden p-0"
